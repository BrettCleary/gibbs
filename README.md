...
apps/web/db/schema/   Drizzle schema (source of truth) · apps/web/migrations/  drizzle-kit output
apps/web/supabase/    local Supabase project config
```
...
table plus `relations.ts`); migrations are generated by drizzle-kit into
**`apps/web/migrations/`** and applied to Supabase Postgres. The Python API
...
# schema workflow
pnpm --filter @alloylab/web db:generate  # npx drizzle-kit generate -> apps/web/migrations/NNNN_*.sql
pnpm --filter @alloylab/web db:migrate   # apply to DATABASE_URL
pnpm --filter @alloylab/web db:studio    # browse
